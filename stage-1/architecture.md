...
**Which process decides.** The router, in `uavx_comms/yield_rule.py`. `uavx_comms` already depends on `uavx_mission` for the frame conversion, so the mission executor cannot import the codec to decode a beacon without a cycle colcon refuses to order, and the router is the node that decodes `HELLO` anyway. It holds its own live position, every neighbour's, and the beacon timestamps the predictor extrapolates from. The decision reaches the node that owns the aircraft's motion on `/<vehicle>/yield_hold`, this vehicle's own namespace carrying no `SwarmPacket`, which is the shape `/<vehicle>/role_slot` already uses. It is published every tick and not on a change, because one dropped edge would leave the aircraft flying into a conflict or stopped for the rest of the run.

**A held vehicle predicts at the velocity it would resume with.** This is the part that is easy to get wrong and expensive to get wrong. A stopped vehicle has no closing speed, so a rule asking whether it is in conflict right now answers no the moment it acts: it releases, moves back into the conflict, holds again, and creeps through the crossing at a fraction of cruise. That run reports a yield event and a hold above zero and satisfies every requirement the gate has, while doing the one thing the rule exists to prevent. The question a held vehicle keeps asking is whether resuming would still be unsafe.

**Holding is the track clock stopping.** The commanded point is `track.position_at(t)` and a hold freezes `t` rather than commanding somewhere else, so the vehicle stays where it is and then carries on along the same line from the same place. The leg after the hold is the same leg, later by exactly the length of the hold, which is why a vehicle that gave way still flies its full 240 m. `vehicles_completed` is the guard on that: a vehicle that holds and never releases has one yield event and a long hold, and is safe by every other number in the record.

**Starting together.** Both legs are 240 m and both begin at `t = 20 s`, so neither vehicle reaches the crossing first and the safe outcome cannot come from one of them happening to be late. The executors are launched during bring-up and hold at the heads of their lines until the runner tells them where the scenario's zero sits, which nothing knows until the ingress has finished. All of them are given the same instant in the simulated clock they share, because a pair that started a parameter call apart would pass safely with the rule doing nothing.
